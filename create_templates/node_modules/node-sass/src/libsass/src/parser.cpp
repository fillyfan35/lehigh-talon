...
// prominently exhibited by the pision operation, since it is not
...
namespace Sass {
  ...
  // can come first in the whole selector sequence (like `> p').
  ...
  Expression_Obj Parser::parse_operators()
  {
    ...
    while (lex_css< class_char< static_ops > >()) {
      ...
      switch(*lexed.begin) {
        ...
        case '/': operators.push_back({ Sass_OP::p, left_ws != 0, right_ws != 0 }); break;
  ...
  Expression_Obj Parser::fold_operands(Expression_Obj base, std::vector<Expression_Obj>& operands, std::vector<Operand>& ops, size_t i)
  {
    ...
    for (size_t S = operands.size(); i < S; ++i) {
      ...
      if (b && ops[i].operand == Sass_OP::p && b->left()->is_delayed() && b->right()->is_delayed()) {
