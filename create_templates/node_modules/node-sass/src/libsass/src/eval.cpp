...
namespace Sass {
  ...
  Expression_Ptr Eval::operator()(Binary_Expression_Ptr b_in)
  {
    ...
    if (!force && op_type == Sass_OP::p && b->is_delayed()) {
      ...
      if (Number_Ptr r_n = Cast<Number>(rhs)) {
        try {
          switch (op_type) {
            ...
            case Sass_OP::ADD: case Sass_OP::SUB: case Sass_OP::MUL: case Sass_OP::p: case Sass_OP::MOD:
    ...
    if (String_Schema_Ptr s_r = Cast<String_Schema>(b->right())) {
      ...
      if (!s_r->has_interpolant() && (!s_r->is_left_interpolant() || op_type == Sass_OP::p)) {
        ret_schema = SASS_MEMORY_NEW(String_Schema, b->pstate());
        Binary_Expression_Obj bin_ex = SASS_MEMORY_NEW(Binary_Expression, b->pstate(),
    ...
    {
      if (op_type == Sass_OP::p || op_type == Sass_OP::MUL || op_type == Sass_OP::MOD || op_type == Sass_OP::ADD || op_type == Sass_OP::SUB ||
          op_type == Sass_OP::EQ) {
    ...
    try {
      ...
      if (l_type == Expression::NUMBER && r_type == Expression::NUMBER) {
        ...
        // if (op_type == Sass_OP::p) interpolant = true;
