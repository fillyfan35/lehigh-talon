...
declare class AggregateError<T extends Error = Error> extends Error implements Iterable<T> {
	...
	@returns An Error that is also an [`Iterable`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Iterators_and_Generators#Iterables) for the inpidual errors.
	...
	for (const inpidualError of error) {
		console.log(inpidualError);
	}
	//=> [Error: foo]
	//=> [Error: bar]
	//=> [Error: baz]
